...
#define SIZE_OF_VARIABLES 2048
#define TOTAL_TIME 131072				//This naive algorithm is O(t) and Memory-limited
...
void time_evolution(double* chain){
	...
	for(int i=1;i<TOTAL_TIME;i++){
		for(int j=0;j<SIZE_OF_VARIABLES;j++){
			XX[j]=*(chain+(i-1)+j*TOTAL_TIME);
		}
		int u=rand()%SIZE_OF_VARIABLES;
		XX[u]=new_value_of(*(chain+(i-1)+u*TOTAL_TIME));
